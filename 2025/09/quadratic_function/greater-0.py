...
def create_positive_definite_quadratic():
    """
    創建一個正定二次型函式的碗形狀圖
    """
    # 設定係數，確保 f 始終為正
    ...
    print(f"由於判別式 < 0 且 a > 0，函式 f 始終為正")
...
def main():
    """主函數"""
    print("=" * 60)
    print("二次型函式 f = ax² + 2bxy + cy² 的碗形狀繪圖")
    print("=" * 60)

    try:
        # 創建主要的碗形狀圖
        print("\n1. 創建主要碗形狀圖...")
        fig1 = create_positive_definite_quadratic()
        plt.savefig(
            "./quadratic_bowl_main.png",
            dpi=300,
            bbox_inches="tight",
        )
        print("已保存主要圖表為: quadratic_bowl_main.png")
        ...
        print("\n繪圖完成！")
        print("\n數學說明：")
        print("- 當判別式 b² - ac < 0 且 a > 0 時，二次型函式為正定")
