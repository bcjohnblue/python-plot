...
def create_negative_semidefinite_quadratic():
    """
    創建一個半負定二次型函式的倒碗形狀圖
    """
    # 設定係數，確保 f ≤ 0
...
def main():
    """主函數"""
    print("=" * 60)
    print("二次型函式 f = ax² + 2bxy + cy² 的倒碗形狀繪圖 (f ≤ 0)")
    print("=" * 60)

    try:
        # 創建主要的倒碗形狀圖
        print("\n1. 創建主要倒碗形狀圖...")
        fig1 = create_negative_semidefinite_quadratic()
        plt.savefig(
            "./quadratic_inverted_bowl_semidefinite.png",
            dpi=300,
            bbox_inches="tight",
        )
        print("已保存主要圖表為: quadratic_inverted_bowl_semidefinite.png")
        ...
        print("\n繪圖完成！")
        print("\n數學說明：")
        print("- 當判別式 b² - ac ≤ 0 且 a ≤ 0 時，二次型函式為半負定")
